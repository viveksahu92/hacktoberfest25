...
st.markdown(f"**Data Period:** {start_date.strftime('%B %d, %Y')} to {end_date.strftime('%B %d, %Y')}")
...
with col4:
    correlation_avg = filtered_log_returns.corr().values[np.triu_indices_from(filtered_log_returns.corr().values, k=1)].mean()
...
with tab2:
    ...
    volume_data = pd.DataFrame({
        ticker: volumes.get(ticker, pd.Series()) 
        for ticker in selected_stocks
    }).dropna(how='all')
    ...
    for idx, ticker in enumerate(selected_stocks):
        with cols[idx % 2]:
            ...
            if len(returns_data) > 0:
                sns.histplot(returns_data, kde=True, stat="density", ax=ax, label="Empirical", alpha=0.6)
                
                # Normal distribution overlay
                normal_data = np.random.normal(
                    loc=returns_data.mean(), 
                    scale=returns_data.std(), 
                    size=1000
                )
...
with tab4:
    ...
    with col1:
        ...
        pe_sector_data = pd.DataFrame({
            ...
            "Sector": [sectors.get(ticker, 'Unknown') for ticker in selected_stocks]
        })
    ...
    with col2:
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=10, color=annual_returns, colorscale='RdYlGn', showscale=True),
    ...
    if (show_distance_matrix or show_mds or show_kmeans) and len(selected_stocks) > 1:
        ...
        if len(selected_stocks) >= 2:
            ...
            if show_mds:
                ...
                fig, ax = plt.subplots(figsize=(12, 10))
                scatter = ax.scatter(mds_results[:, 0], mds_results[:, 1], c='orange', s=100, alpha=0.6)
...
with tab5:
    ...
    with col5:
        ...
        summary_stats['Annual Volatility %'] = filtered_log_returns.std() * np.sqrt(252) * 100
